...
#[tokio::test]
async fn test_consistency_all_sled() -> anyhow::Result<()> {
    let _ = tracing_subscriber::fmt::try_init();
    ...
    let [node_1, node_2, node_3] =
        connect_sled_cluster([node_1_addr, node_2_addr, node_3_addr]).await;
...
#[tokio::test]
async fn test_consistency_none_sled() -> anyhow::Result<()> {
    let _ = tracing_subscriber::fmt::try_init();
...
#[tokio::test]
async fn test_async_operations_sled() -> anyhow::Result<()> {
    let _ = tracing_subscriber::fmt::try_init();
...
// line `connect_cluster` but uses sled storage instead
async fn connect_sled_cluster(
    addrs: [SocketAddr; 3],
